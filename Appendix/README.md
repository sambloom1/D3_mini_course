...
| Generators| For more complex visuals using d3 specific functions, it helps to create a const naming it a generator. Which can then be passed when appending an element. For a line graph this would be a line generator where we pass the methods and the arguments of d3.line() then when appropriate we call the generator and pass the dataset as the argument for the generator |  const lineGenerator = d3.line() <br> .x(arguments for x co-ordinated, usually an xScale) <br>
.y(arguments for the y values for the line generator of .line(), usually a yScale
| d3.line() and d3.area() | d3.line() and d3.area() are d3 specific functions. This is where d3 excels in utility as complicated mathematics/formula to detemrine how a line is plotted form data to the element, is abstracted to a d3 specific function
|
